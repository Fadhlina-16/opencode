...
<template>
  <div class="p-4 lg:p-6">
    ...
    <div v-if="error" class="rounded-xl border border-border bg-card p-6 text-center">
      <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-error-bg">
        <svg class="h-6 w-6 text-error-text" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
        </svg>
      </div>
      <h2 class="text-lg font-semibold text-heading">Unable to load schedule</h2>
    ...
    <div v-else-if="loading">
      <div class="rounded-xl border border-border bg-card p-4 lg:p-5">
        <div class="mb-4 h-8 w-48 animate-pulse rounded bg-skeleton" />
        <div class="mb-6 grid grid-cols-7 gap-1">
          <div v-for="i in 7" :key="i" class="h-4 animate-pulse rounded bg-skeleton" />
        </div>
        <div class="grid grid-cols-7 gap-1">
          <div v-for="i in 35" :key="i" class="aspect-square animate-pulse rounded bg-subtle" />
        </div>
      </div>
    </div>
    ...
    <template v-else>
      <div class="mb-5 flex flex-wrap items-center gap-2">
        <button
          ...
          :class="activeFilter === filter
            ...
            : 'border border-border bg-card text-body hover:bg-accent hover:text-white'"
      ...
      <div class="grid gap-6 lg:grid-cols-5">
        <section class="rounded-xl border border-border bg-card p-4 lg:col-span-3 lg:p-5">
          <div class="mb-4 flex items-center justify-between">
            <button
              type="button"
              class="inline-flex items-center justify-center rounded-lg p-2 text-body transition hover:bg-subtle"
              @click="prevMonth"
            ...
            <h2 class="text-lg font-semibold text-heading lg:text-xl">{{ monthLabel }}</h2>
            <button
              type="button"
              class="inline-flex items-center justify-center rounded-lg p-2 text-body transition hover:bg-subtle"
              @click="nextMonth"
          ...
          <div class="mb-1 grid grid-cols-7">
            <div
              ...
              :key="day"
              class="py-1 text-center text-xs text-muted"
            >
              {{ day }}
            </div>
          </div>
          ...
          <div class="grid grid-cols-7">
            ...
            >
              <div
                ...
                :class="[
                  cell.isToday && !isSelected(cell.date) ? 'border border-accent font-semibold text-accent' : '',
                  isSelected(cell.date) ? 'bg-accent font-semibold text-white' : '',
                   cell.isCurrentMonth && !isSelected(cell.date) ? 'text-body hover:bg-subtle' : '',
                  !cell.isCurrentMonth && !isSelected(cell.date) ? 'text-muted' : '',
                ]"
              >
                {{ cell.day }}
              </div>
        ...
        <section class="rounded-xl border border-border bg-card p-4 lg:col-span-2 lg:p-5">
          ...
          <template v-else-if="selectedSessions && selectedSessions.length === 0">
            <div class="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p class="mt-1 text-xs text-muted">Nothing on {{ selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) }}</p>
            </div>
          </template>
          <template v-else>
            <header class="mb-4">
              <h2 class="text-sm font-semibold text-heading">{{ selectedDate.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' }) }}</h2>
              <p class="mt-0.5 text-xs text-muted">{{ selectedSessions.length }} item{{ selectedSessions.length !== 1 ? 's' : '' }}</p>
            </header>
            <div class="space-y-3">
              <div
              ...
              >
                <div class="mb-1">
                  <span
                    ...
                    :class="{
                      'bg-type-lecture-bg text-type-lecture-text': session.type === 'lecture',
                      'bg-type-quiz-bg text-type-quiz-text': session.type === 'quiz',
                      'bg-type-assignment-bg text-type-assignment-text': session.type === 'assignment',
                      'bg-type-session-bg text-type-session-text': session.type === 'session',
                    }"
                  >
                    {{ session.type }}
                  </span>
                </div>
                <h4 class="text-sm font-semibold text-heading">{{ session.module }}</h4>
                <div class="mt-1 flex items-center justify-between">
                  <p v-if="session.student" class="flex items-center gap-1 text-sm text-body">
                  ...
                  <div v-if="session.duration" class="text-right">
                    <p class="text-sm text-body">{{ formatTime(session.date) }}</p>
